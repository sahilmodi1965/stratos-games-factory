...
11. **Runtime-smoke in `npm run validate` (#33).** Every game's `npm run validate` MUST depend on a runtime-smoke step that executes the game's primary runtime entry and asserts a positive observable outcome. The smoke runs in CI, not just on Ripon's device — if it's missing, add it. Spec + per-kind pattern: [`templates/runtime-smoke-spec.md`](templates/runtime-smoke-spec.md). Examples: arrow-puzzle calls `generateLevel(20, getDifficulty(20))` and asserts `arrows.length >= 15` + fill density; house-mafia creates a room and asserts a 6-char room code + websocket open; Bloxplode boots the shell and asserts canvas initializes with non-zero block count. If your change touches the runtime entry or any file it transitively calls, re-run `npm run validate` locally before pushing — a green local build is the contract with Ripon. Reference: arrow-puzzle PRs #125 + #126 both shipped green while completely broken — Ripon caught both on device. The smoke is the gate that flips "build-green = feature-working" from aspiration to enforcement.
```
