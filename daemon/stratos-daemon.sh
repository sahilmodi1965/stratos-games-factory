...
\`\`\`" >/dev/null 2>&1 || true
    ...
    # PR body follows the same template as .github/pull_request_template.md
    # so daemon PRs and human PRs look identical to reviewers. Claude's
    # summary fills "What changed"; the file list fills "What to look for".
    changed_files_list=$(git diff --name-only "origin/$default_branch"...HEAD 2>/dev/null \
      | head -15 \
      | sed 's/^/- `/' \
      | sed 's/$/`/')
    [[ -z "$changed_files_list" ]] && changed_files_list="- _(no files detected)_"
...
4. [ ] Start a game — does the new feature / fix work as described in the issue?
...
This PR changes the following file(s) — focus your testing on the behaviors they govern:
...
Closes #${num}
🤖 Generated by the Stratos Games Factory builder agent.
EOF
